...
function renderCreditOptions(){
  const sel=$('creditCustomer');
  const current=sel.value;
  sel.innerHTML='<option value="">Choisir…</option>'+customers.filter(c=>c.active).map(c=>`<option value="${c.id}">${esc(c.full_name)} (${money(c.balance_cents)})</option>`).join('');
